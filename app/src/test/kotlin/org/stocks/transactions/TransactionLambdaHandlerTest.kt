...
class TransactionLambdaHandlerTest {
    ...
    fun `should handle valid POST request and return stock response with dividends`() {
        ...
        whenever(financialModelingService.getStockPrice("PEP", context)).thenReturn(BigDecimal.ONE)
        whenever(financialModelingService.getDividends("PEP", context)).thenReturn(listOf(dividend))
